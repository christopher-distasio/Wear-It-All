...
const UpdateForm = ({ imageData }) => {
    ...
    const handleSubmit = async (event) => {
        event.preventDefault();
        try {
            const response = await fetch(`https://wearitagain.onrender.com/items/${imageData.item_id}`, {
                method: 'PUT',
                headers: {
                ...
                <Form onSubmit={handleSubmit}>
                    <Form.Group controlId="formImage">
                        ...
                        <div className="image-upload-field" onDrop={handleDrop} onDragOver={handleDragOver}>
                            {image ? <img src={image} alt="Uploaded" style={{ maxWidth: '100%' }} /> : <p className="hidden">Drag and Drop image here to upload into inventory</p>}
                        </div>
